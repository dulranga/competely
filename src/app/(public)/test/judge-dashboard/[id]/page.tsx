...
export default function JudgeReviewPage() {
    ...
    const chartData = useMemo(() => {
        return CRITERIA.map((c) => ({
            ...
            max: c.max,
            color: (submission.marks[c.id] || 0) > 7 ? "#6dd594" : (submission.marks[c.id] || 0) > 4 ? "#e5ab7d" : "#ff5f56"
        }));
    }, [submission.marks]);
            ...
            <header className="px-6 py-4 bg-white border-b border-[#e8e2de] flex items-center justify-between shrink-0 z-20">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-right mr-4">
                        <div className="text-xs font-black text-[#0c0803]/30 uppercase tracking-widest">Total Score</div>
                ...
                <div className="flex-1 bg-[#fbf6f3] flex flex-col relative border-r border-[#e8e2de]">
                    ...
                    <div className="h-12 bg-white/80 backdrop-blur-sm border-b border-[#e8e2de] flex items-center justify-between px-4 sticky top-0 z-10">
                        ...
                        <div className="flex items-center gap-1 bg-white border border-[#e8e2de] rounded-lg p-1 shadow-sm">
                            <button onClick={handleZoomOut} className="p-1.5 hover:bg-[#fbf6f3] rounded-md text-[#0c0803]/60">
                                <ZoomOut size={16} />
                            </button>
                            <span className="text-xs font-mono w-12 text-center text-[#0c0803]/60">
                                {Math.round(zoomLevel * 100)}%
                            </span>
                            <button onClick={handleZoomIn} className="p-1.5 hover:bg-[#fbf6f3] rounded-md text-[#0c0803]/60">
                                <ZoomIn size={16} />
                            </button>
                            <div className="w-px h-4 bg-[#e8e2de] mx-1" />
                            <button onClick={handleResetZoom} className="p-1.5 hover:bg-[#fbf6f3] rounded-md text-[#0c0803]/60" title="Reset Zoom">
                                <RotateCcw size={14} />
                            </button>
                        </div>
                    </div>
                        ...
                        >
                            ...
                            <div className="flex flex-wrap gap-2 mb-8">
                                {submission.techStack.map(tech => (
                                    <span key={tech} className="px-3 py-1 bg-[#fbf6f3] border border-[#e8e2de] rounded-lg text-xs font-bold text-[#0c0803]/60">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                            ...
                            <div className="flex flex-col gap-4 mt-12 pt-8 border-t border-[#e8e2de]">
                                <h3 className="text-sm font-black uppercase tracking-widest text-[#0c0803]/40">Links</h3>
                                <div className="flex gap-4">
                                    <a href="#" className="flex items-center gap-2 text-[#0c0803] font-bold hover:text-[#e5ab7d] transition-colors">
                        ...
                        <div className="h-48 w-full bg-[#fbf6f3] rounded-2xl p-4 border border-[#e8e2de]">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={chartData} barSize={20}>
                                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#0c0803' }} />
                                    <Tooltip
                            ...
                            {CRITERIA.map((criterion) => (
                                <div key={criterion.id} className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <label className="font-bold text-sm text-[#0c0803]">{criterion.label}</label>
                                        ...
                                        <div className={cn(
                                            ...
                                            {submission.marks[criterion.id]} <span className={cn("text-xs", isLocked ? "text-[#0c0803]/20" : "text-white/50")}>/ {criterion.max}</span>
                                        </div>
                                    </div>
                                    <Slider
