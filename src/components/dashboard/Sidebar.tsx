...
export const DashboardSidebar: FC<DashboardSidebarProps> = ({ items, overriddenSecondary }) => {
    ...
    return (
        <div className="flex h-screen bg-background overflow-hidden select-none">
            {/* Left Static Sidebar */}
            <div className="w-20 flex flex-col items-center py-8 pb-10 bg-background/40 backdrop-blur-xl border-r border-border/60 z-20">
                                ...
                                <Tooltip key={item.href}>
                                    <TooltipTrigger asChild>
                                        ...
                                        >
                                            ...
                                            />
                                            {isActive && (
                                                <div className="absolute -left-1 w-1 h-4 bg-foreground rounded-full" />
                                            )}
                                        </Link>
                                    </TooltipTrigger>
                ...
                <div className="px-3 w-full pt-6 border-t border-border/40">
                    <Button
                        variant="competely"
                        size="icon-lg"
            ...
            {hasSecondary && (
                <div className="w-72 flex flex-col h-full pl-2 relative z-10 transition-all duration-300">
                    <div className="flex-1 overflow-hidden py-4 pr-3">
                        <div className="h-full bg-background/60 backdrop-blur-md rounded-[2.5rem] shadow-sm border border-border/40 flex flex-col overflow-hidden relative">
