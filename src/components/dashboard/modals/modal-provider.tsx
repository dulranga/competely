...
import { modalRegistry, type ModalContextType, type ModalName, type OpenModalFunction } from "./modal-registry";
...
export function ModalProvider({ children }: { children: ReactNode }) {
    const [modals, setModals] = useState<(ModalContextType<ModalName> & { id: string })[]>([]);
    ...
    const openModal = useCallback<OpenModalFunction>((name, data) => {
        ...
        requestAnimationFrame(() => {
            const isAvailableAction = Object.prototype.hasOwnProperty.call(modalRegistry, name);
    ...
    return (
        <ModalContext.Provider value={{ modals, openModal, closeModal, currentModalId, closeAllModals }}>
            {children}
            {modals.map((modal) => (
                <ModalRenderer key={modal.id} modal={modal} modalId={modal.id} closeModal={closeModal} />
            ))}
