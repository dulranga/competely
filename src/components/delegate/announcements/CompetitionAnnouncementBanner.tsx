...
import { getLatestAnnouncementByCompetitionAction } from "~/data-access/announcements/actions";
import AnnouncementsModal from "./AnnouncementsModal";
import { cn } from "~/lib/utils";

interface CompetitionAnnouncementBannerProps {
    competitionId: string;
    competitionName?: string;
}

export default function CompetitionAnnouncementBanner({
    competitionId,
    competitionName,
}: CompetitionAnnouncementBannerProps) {
    const [isModalOpen, setIsModalOpen] = useState(false);

    const { data: latestAnnouncement, isLoading } = useQuery({
        queryKey: ["latest-announcement", competitionId],
        queryFn: () => getLatestAnnouncementByCompetitionAction(competitionId),
    });
    ...
    return (
        <>
            ...
            {/* Note: In a real app we might want a filtered history modal specifically for this competition */}
