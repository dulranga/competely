...
- **STRICT COLOR POLICY: NEVER use hard-coded hex colors, RGB, or named colors (e.g., `#ffffff`, `white`, `red-500`) in UI components or styles. You must use CSS variables or Tailwind utility classes that reference the theme (e.g., `bg-primary`, `bg-background`, `bg-input-background`, `text-foreground`). This is non-negotiable.**
